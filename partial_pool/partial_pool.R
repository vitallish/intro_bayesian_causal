...
####------------------------ Sample Posterior    ---------------------------####
partial_pool_model = stan_model(file = here::here("partial_pool", "partial_pool.stan"))

stan_res = sampling(partial_pool_model, data = stan_data,
                    pars = c("odds_ratio", "overall_odds_ratio", "overall_odds_ratio_nw",
                             "odds_ratio_nw"),
                    warmup = warmup, iter = iter, chains=1, seed=1)

Psi_draws = extract(stan_res, pars='odds_ratio')[[1]]
Psi_draws_nw = extract(stan_res, pars='odds_ratio_nw')[[1]]

overall_mean = extract(stan_res, pars='overall_odds_ratio')[[1]]
overall_mean_nw = extract(stan_res, pars='overall_odds_ratio_nw')[[1]]
...
for(vf in 1:5){
  ...
  # this is the same as creating the counterfactual right?
...
# png(here("partial_pool","ppooling_plot.png"), width = 600, height = 500)
...
# Distribution sems the same for overall OR
plot(density(overall_mean), main = "Comparing Weighted and Unweighted\nOverall OR")
lines(density(overall_mean_nw), col = "red")

# This applies small sample sizes (like for Psi 5) as well
plot(density(Psi_draws[,5]), main = "Comparing Weighted and Unweighted\nPsi 5 OR")
lines(density(Psi_draws_nw[, 5]), col = "red")


summary(Psi_draws)
summary(Psi_draws_nw)
